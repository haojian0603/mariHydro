...
mod tests {
    ...
    #[inline]
    pub fn wind_drag_coefficient_lp81(wind_speed_10m: f64) -> f64 {
        let w = wind_speed_10m.abs().min(100.0);
    ...
    pub fn compute_pressure_gradient_unstructured(
        pressure: &[f64],
        ...
        assert_eq!(u_acc.len(), mesh.n_cells);
        assert_eq!(v_acc.len(), mesh.n_cells);
        ...
        grad_p
            .par_iter()
            .zip(u_acc.par_iter_mut())
            .zip(v_acc.par_iter_mut())
            .zip(&mesh.cell_area)
            .for_each(|(((grad, u), v), &area)| {
