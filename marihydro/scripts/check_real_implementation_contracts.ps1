...
try {
    ...
    $patternChecks = @(
        ...
        @{ Name = "tide filename heuristic dispatch"; Path = "crates/mh_io/src/netcdf_tide.rs"; Pattern = 'let model = TidalModel::detect\(path\)'; Message = "tide reader dispatch must use the real path type and layout instead of filename heuristics when the path exists" },
        @{ Name = "tide synthetic leading dimension fallback"; Path = "crates/mh_io/src/netcdf_tide.rs"; Pattern = 'dims\.first\(\)\.copied\(\)\.unwrap_or_default\(\)\s*==\s*1'; Message = "netcdf_tide must not treat a missing leading axis as an implicit singleton dimension" },
        @{ Name = "tide public heuristic detector"; Path = "crates/mh_io/src/netcdf_tide.rs"; Pattern = 'pub fn detect\(path: &Path\)'; Message = "tide reader must not expose a public filename-heuristic detector as part of the mainline surface" },
        ...
        @{ Name = "geo geographic-target input bypass"; Path = "crates/mh_geo/src/transform.rs"; Pattern = 'if self\.is_identity \|\| self\.target_crs\.is_geographic\(\)\s*\{\s*return Ok\(0\.0\);'; Message = "GeoTransformer must still validate projected input when the target CRS is geographic" },
        @{ Name = "geo projection synthetic NaN fallback"; Path = "crates/mh_geo/src/projection/transverse_mercator.rs"; Pattern = 'unwrap_or\(f64::NAN\)'; Message = "projection helpers must return explicit errors instead of synthetic NaN sentinels" },
        @{ Name = "utm helper synthetic scalar surface"; Path = "crates/mh_geo/src/projection/utm.rs"; Pattern = 'pub fn utm_scale_factor\(.*\) -> f64|pub fn utm_convergence_angle\(.*\) -> f64'; Message = "UTM helper APIs must return explicit errors instead of plain f64 sentinel surfaces" },
        @{ Name = "web mercator synthetic zero fallback"; Path = "crates/mh_geo/src/projection/web_mercator.rs"; Pattern = 'unwrap_or\(\(0\.0, 0\.0\)\)'; Message = "Web Mercator helpers must not collapse impossible failures into synthetic zero coordinates" },
        @{ Name = "runtime synthetic NUMA memory defaults"; Path = "crates/mh_runtime/src/numa.rs"; Pattern = '8 \* 1024 \* 1024 \* 1024|4 \* 1024 \* 1024 \* 1024'; Message = "runtime NUMA detection must not fabricate fixed memory sizes" },
        @{ Name = "runtime silent NUMA metadata parse"; Path = "crates/mh_runtime/src/numa.rs"; Pattern = 'read_to_string\(&cpulist_path\)\.unwrap_or_default\(\)|read_to_string\(&meminfo_path\)\.unwrap_or_default\(\)|parse\(\)\.unwrap_or\(0\)|parse::<u64>\(\)\.unwrap_or\(0\)'; Message = "runtime NUMA detection must not silently swallow system metadata errors" },
        ...
        @{ Name = "gdal synthetic band layout"; Path = "crates/mh_io/src/drivers/gdal/driver.rs"; Pattern = 'unwrap_or\(&empty_bands\)|bands\.len\(\)\.max\(1\)'; Message = "gdalinfo metadata parsing must not invent a synthetic one-band layout when bands metadata is missing" },
        ...
        @{ Name = "netcdf header placeholder parsing"; Path = "crates/mh_io/src/drivers/netcdf/driver.rs"; Pattern = 'parts\.next\(\)\.unwrap_or\(\"\"\)|if let Some\(space\) = cleaned\.find\('; Message = "ncdump header parsing must not ignore malformed variable declarations or attributes by inserting empty placeholders" },
        ...
        @{ Name = "netcdf time tolerant public parser"; Path = "crates/mh_io/src/drivers/netcdf/time.rs"; Pattern = 'pub fn parse_calendar_or_default\('; Message = "CF time module must not expose a public parser that silently defaults malformed calendars" },
