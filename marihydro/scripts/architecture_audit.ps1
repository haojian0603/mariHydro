...
try {
    ...
    Invoke-InformationalScan -Name 'geo auto-zone clamp/default residue' -Roots @("crates/mh_geo/src") -Pattern 'return 39;|zone\.clamp\(25,\s*45\)|zone\.clamp\(13,\s*23\)|zone\.clamp\(1,\s*60\)|\(\(lon \+ 180\.0\) / 6\.0\)\.floor\(\) as (?:u8|i32) \+ 1'
