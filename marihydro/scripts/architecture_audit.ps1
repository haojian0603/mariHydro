...
try {
    ...
    if (-not (Invoke-GuardStep -Name "check_io_snapshot_contracts.ps1" -Path (Join-Path $ScriptDir "check_io_snapshot_contracts.ps1") -Arguments @{})) {
        $Failed += "check_io_snapshot_contracts.ps1"
    }
    ...
    if (-not (Invoke-GuardStep -Name "check_geo_vector_contracts.ps1" -Path (Join-Path $ScriptDir "check_geo_vector_contracts.ps1") -Arguments @{})) {
        $Failed += "check_geo_vector_contracts.ps1"
    }

    if (-not (Invoke-GuardStep -Name "check_geo_central_meridian_contracts.ps1" -Path (Join-Path $ScriptDir "check_geo_central_meridian_contracts.ps1") -Arguments @{})) {
    ...
    if (-not (Invoke-GuardStep -Name "check_geo_crs_contracts.ps1" -Path (Join-Path $ScriptDir "check_geo_crs_contracts.ps1") -Arguments @{})) {
    ...
    if (-not (Invoke-GuardStep -Name "check_mesh_structured_contracts.ps1" -Path (Join-Path $ScriptDir "check_mesh_structured_contracts.ps1") -Arguments @{})) {
        $Failed += "check_mesh_structured_contracts.ps1"
    }

    if (-not (Invoke-GuardStep -Name "check_mhb_contracts.ps1" -Path (Join-Path $ScriptDir "check_mhb_contracts.ps1") -Arguments @{})) {
        $Failed += "check_mhb_contracts.ps1"
    }

    if (-not (Invoke-GuardStep -Name "check_gmsh_contracts.ps1" -Path (Join-Path $ScriptDir "check_gmsh_contracts.ps1") -Arguments @{})) {
        $Failed += "check_gmsh_contracts.ps1"
    }
    ...
    Invoke-InformationalScan -Name 'geo auto-zone clamp/default/single-source residue' -Roots @("crates/mh_geo/src") -Pattern 'return 39;|zone\.clamp\(25,\s*45\)|zone\.clamp\(13,\s*23\)|zone\.clamp\(1,\s*60\)|\(\(lon \+ 180\.0\) / 6\.0\)\.floor\(\) as (?:u8|i32) \+ 1'
    ...
    Invoke-InformationalScan -Name 'MHB binary fallback residue' -Roots @("crates/mh_mesh/src/io/mhb.rs") -Pattern 'to_f(?:32|64)\(\)\.unwrap_or\(0\.0\)|values\.first\(\)\.copied\(\)\.unwrap_or\(0\)|\.map\(\|v\| v as usize\)'
    ...
    Invoke-InformationalScan -Name 'snapshot statistics fallback residue' -Roots @("crates/mh_io/src/snapshot.rs") -Pattern 'pub fn statistics\(&self\) -> StateStatistics|StateStatistics::default\(\)|return StateStatistics::default\(\);'
    ...
    Invoke-InformationalScan -Name 'tide interpolation option-fallback residue' -Roots @("crates/mh_io/src/netcdf_tide.rs") -Pattern 'pub fn interpolation_indices\(.*\) -> Option<InterpolationIndices>|fn wrap_longitude_to_range\(.*\) -> Option<f64>|\.interpolation_indices\(lon, lat\)\s*\.ok_or\('
