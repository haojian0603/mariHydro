...
fn forward_internal(
    ...
    let lonsign = if lon_diff.is_sign_negative() {
        -1.0
    } else {
        1.0
    };
...
///
/// # Errors
/// 如果输入坐标超出投影适用范围，或正向投影计算失败，则返回错误。
pub fn scale_factor_at(params: &TransverseMercatorParams, lon: f64, lat: f64) -> MhResult<f64> {
    forward_internal(params, lon, lat).map(|res| res.k)
}
...
/// # Errors
/// 如果输入坐标超出投影适用范围，或正向投影计算失败，则返回错误。
pub fn convergence_angle(params: &TransverseMercatorParams, lon: f64, lat: f64) -> MhResult<f64> {
    forward_internal(params, lon, lat).map(|res| res.gamma.to_radians())
}
...
mod tests {
    ...
    fn test_scale_factor() {
        ...
        // 中央子午线处
        let k = scale_factor_at(&params, 117.0, 40.0).expect("scale factor failed");
        assert!((k - 0.9996).abs() < 0.0001, "k = {k}");
    }

    #[test]
    fn test_scale_factor_rejects_invalid_latitude() {
        let params = TransverseMercatorParams::utm(50, true);
        assert!(scale_factor_at(&params, 117.0, 95.0).is_err());
    }
}
