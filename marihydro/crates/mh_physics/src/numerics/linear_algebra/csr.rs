...
impl<S: RuntimeScalar> CsrMatrix<S> {
    ...
    /// 矩阵-向量乘法（Backend 缓冲区版本）
    pub fn mul_vec_backend<B: Backend<Scalar = S>>(
        &self,
        x: &B::Buffer<S>,
        y: &mut B::Buffer<S>,
    ) -> Result<(), CsrError> {
        let x = x.try_as_slice().ok_or_else(|| {
            CsrError::BackendAccess("x buffer not accessible".to_string())
        })?;
        let y = y.try_as_slice_mut().ok_or_else(|| {
            CsrError::BackendAccess("y buffer not accessible".to_string())
        })?;
        self.mul_vec(x, y);
        Ok(())
    }
    ...
    /// 矩阵-向量乘法加法（Backend 缓冲区版本）
    pub fn mul_vec_add_backend<B: Backend<Scalar = S>>(
        &self,
        alpha: S,
        x: &B::Buffer<S>,
        y: &mut B::Buffer<S>,
