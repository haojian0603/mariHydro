...
impl ProjectConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.duration_seconds <= 0.0 { return Err("Duration must be positive".into()); }
        if self.output_interval <= 0.0 { return Err("Output interval must be positive".into()); }
        if self.min_depth < 0.0 { return Err("Min depth cannot be negative".into()); }
        Ok(())
    }
}
