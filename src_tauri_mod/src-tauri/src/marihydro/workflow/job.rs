...
impl SimulationJob {
    pub fn new(id: String, project_id: String) -> Self {
        Self {
            ...
            message: Some("Pending".into()), result_path: None, parameter_overrides: None,
    ...
    pub fn start(&mut self) { self.status = JobStatus::Running; self.started_at = Some(Utc::now()); self.message = Some("Running".into()); }
    pub fn update_progress(&mut self, progress: f64, message: Option<&str>) { self.progress = progress.clamp(0.0, 100.0); if let Some(m) = message { self.message = Some(m.into()); } }
    pub fn complete(&mut self, result_path: &str) { self.status = JobStatus::Completed; self.progress = 100.0; self.result_path = Some(result_path.into()); self.finished_at = Some(Utc::now()); self.message = Some("Completed".into()); }
    pub fn fail(&mut self, error: &str) { self.status = JobStatus::Failed; self.message = Some(error.into()); self.finished_at = Some(Utc::now()); }
    pub fn cancel(&mut self) { self.status = JobStatus::Cancelled; self.message = Some("Cancelled".into()); self.finished_at = Some(Utc::now()); }
}
