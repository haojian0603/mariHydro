...
pub struct GeoTransform {
    ...
    pub pixel_width: f64, pub pixel_height: f64,
    pub rotation_x: f64, pub rotation_y: f64,
}

impl GeoTransform {
    pub fn from_gdal(gt: &[f64; 6]) -> Self {
        Self { origin_x: gt[0], pixel_width: gt[1], rotation_x: gt[2], origin_y: gt[3], rotation_y: gt[4], pixel_height: gt[5] }
    }
    pub fn to_gdal(&self) -> [f64; 6] { [self.origin_x, self.pixel_width, self.rotation_x, self.origin_y, self.rotation_y, self.pixel_height] }
    #[inline]
    ...
    pub fn inverse(&self, x: f64, y: f64) -> Option<(f64, f64)> {
        ...
        let dx = x - self.origin_x; let dy = y - self.origin_y;
        Some(((self.pixel_height * dx - self.rotation_x * dy) / det, (-self.rotation_y * dx + self.pixel_width * dy) / det))
    }
    pub fn resolution(&self) -> (f64, f64) { (self.pixel_width.abs(), self.pixel_height.abs()) }
}

#[derive(Debug, Clone, Default)]
pub struct RasterMetadata {
    pub width: usize, pub height: usize, pub n_bands: usize,
    pub crs_wkt: Option<String>, pub geo_transform: Option<GeoTransform>, pub no_data_value: Option<f64>,
}

impl RasterMetadata { pub fn total_pixels(&self) -> usize { self.width * self.height } }
